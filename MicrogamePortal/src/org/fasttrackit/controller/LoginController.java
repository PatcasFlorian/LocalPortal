...
if(count==0) {
	...
	public ModelAndView listareUseri(@ModelAttribute("myUser") User myUser ,Model model,BindingResult result) throws SQLException, ServletException, IOException {
		setMyUserName(myUser.getFullName());
		System.out.println("MyUserName "+getMyUserName());
		...
		model.addAttribute("myUser", myUser.getFullName());
	...
	public ModelAndView listareUser(@ModelAttribute("user") User user,Model model,BindingResult result) throws SQLException, ServletException, IOException {
		...
		user = adao.getOneUser(user.getFullName());
		user.setId(1);
	...
	//editare User/role
		...
		//redirectionare spre pagina comuna
				...
				//vizualizare toate anunturi Vanzare
				@RequestMapping(value="view-all-sale.htm")
				public ModelAndView viewAllAnnoncementSale() throws SQLException {
					ModelMap model = new ModelMap();
					if((isLoginSuperAdmin() == true)&&(getMyUserName()!=null))	{
						SaleDAO sdao = new SaleDAO();
						UserDAO udao = new UserDAO();
						...
						user = udao.getOneUser(getMyUserName());
						ArrayList<Annoncement> listAnunt = new ArrayList<Annoncement>();
						listAnunt = sdao.getAllAnuncement();
						model.addAttribute("user",user);
						model.addAttribute("listAnunt", listAnunt);
					return new ModelAndView("WEB-INF/Home/view-all-sale.jsp","model",model);
					}
					else {
						isLoginSuperAdmin = false;
						 return new ModelAndView("redirect:/login-portal.htm");
				...
				//editare User/role
				@RequestMapping(value="view-annoncement.htm")
				public ModelAndView viewAnnoncement(@ModelAttribute("id") Annoncement anunt,ModelAndView model,BindingResult result) throws SQLException, IOException {
				
					SaleDAO sdao =new SaleDAO();
					anunt =sdao.getAnuncementId(anunt.getId());
					System.out.println("Id anunt "+anunt.getId());
					model.addObject("anunt", anunt);
					return new ModelAndView("WEB-INF/Home/view-annoncement.jsp","model",model);
				}
				...
				public void getStudentPhoto(HttpServletResponse response, @PathVariable("id") int id) throws Exception {
					...
					SaleDAO sdao =new SaleDAO();
					Annoncement anunt =sdao.getAnuncementId(id);
					

					byte[] bytes = anunt.getImg1();
